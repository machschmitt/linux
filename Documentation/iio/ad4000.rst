...
Omit both ``adi,sdi-pin`` and ``adi,cnv-pin`` properties in device tree to
select this mode.
...
Set the ``adi,sdi-pin`` device tree property to ``"high"`` and omit
``adi,cnv-pin`` to select this mode.
...
Set ``adi,sdi-pin`` to ``"high"`` and ``adi,cnv-pin`` to ``"gpio"`` to select
this mode.
...
Set ``adi,sdi-pin`` to ``"cs"`` and ``adi,cnv-pin`` to ``"gpio"`` to select this
mode.
