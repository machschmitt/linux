 ...
 *
 * A nasty hack to filter out part of the sample data that comes from FPGA IPs.
 *
 * ad4630_fmc
 * HDL project IP's push out
 * to DMA
 *
 * The reasonable parts of this are based on Lars-Peter's
		...
		const struct dmaengine_result *result)
{
	...
	pr_info("INDUSTRIALIO-BUFFER-DMAENGINE-FILTERED\n");
	pr_info("%s: n: %u \n", __func__, n);
	 ...
	 */
	pr_info("%s: sizeof(typeof(addr)): %u \n", __func__, sizeof(typeof(addr)));
	pr_info("%s: bytes_per_datum: %u \n", __func__, queue->buffer.bytes_per_datum);
	pr_info("%s: length: %u \n", __func__, queue->buffer.length);
	pr_info("%s: scan_mask: %lu \n", __func__, queue->buffer.scan_mask[0]);
	pr_info("%s: channel_mask: %lu \n", __func__, queue->buffer.channel_mask[0]);
	addr = block->vaddr + queue->fileio.pos;
	//for (i = 0; i < n / 2; i++) { // noisy waveform
	//	memcpy((addr + i), (addr + (i * 2)), sizeof(typeof(addr)));
	//}
	//for (i = 0; i < n / 2; i = i + 4) { //similar to no discarding
	//	memcpy((addr + i), (addr + i), sizeof(typeof(addr)));
	//	memcpy((addr + i + 1), (addr + i + 1), sizeof(typeof(addr)));
	//}
	//for (i = 0; i < n; i = i + 2) { // noisy waveform
	//	memcpy((addr + (i / 2)), (addr + i), sizeof(typeof(addr)));
	//}
	//u32 data_element;
	//data_element = 0x0000FFFF & *(u32 *)(addr + 0);

	//for (i = 0; i < n; i = i + 2) { // noisy waveform
	//	data_element = 0xFF & *(u32 *)(addr + i);
	//	memcpy((addr + (i / 2)), (addr + i), sizeof(typeof(u16)));
	//}
	unsigned int num_bytes_per_element;
	//num_bytes_per_element = 4; // 4 bytes per data element
	...
	//for (i = 0; i < 40; i = i + num_bytes_per_element) {
	//	//data_element = 0x0000FFFF & *(u32 *)(addr + i);
	//	data_element = *(u32 *)(addr + i);
	//	pr_info("%s: data_element: 0x%08X \n", __func__, data_element);
	//}

	//for (i = 0; i < (num_data_elements / 2); i = i + num_bytes_per_element) { // some good data
	//	data_element = 0x0000FFFF & *(u32 *)(addr + (i * 2));
	//	memcpy((addr + i), &data_element, sizeof(typeof(u32)));
	//}
	//for (i = 0; i < n; i = i + num_bytes_per_element) { // good data! middle buffer glitch
	...
	pr_info("%s: MMAP bytes_used: %u \n", __func__, block->block.bytes_used);
#else
	block->bytes_used -= result->residue;
	block->bytes_used /= 2;
	pr_info("%s: bytes_used: %u \n", __func__, block->bytes_used);
#endif
