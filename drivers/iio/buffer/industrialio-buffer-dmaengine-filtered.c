 ...
 *
 * The reasonable parts of this are based on (copied from) Lars-Peter's
						     ...
						     const struct dmaengine_result *result)
{
	...
	unsigned long flags;
	/*
	 * Hack to discard ADAQ4216 second channel data.
	 ...
	 * coming from DMA is used for single channel ADCs such as ADAQ4216.
	 */
	...
	/*
	 * Each ADC sample is a 32-bit data element.
	 * Modify the DMA block memory to copy over one value every each 4 bytes.
	 * In memory, the ADC data is arrange the following way:
	...
	for (i = 0; i < (n / 2); i = i + queue->buffer.bytes_per_datum)
		memcpy((addr + i), (addr + (i * 2)), queue->buffer.bytes_per_datum);
...
static struct iio_buffer *iio_dmaengine_buffer_alloc(struct dma_chan *chan)
{
	...
	iio_dma_buffer_init(&dmaengine_buffer->queue, chan->device->dev,
			    &iio_dmaengine_filtered_default_ops);
