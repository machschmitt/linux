...
static int ad4134_get_ADC_count(struct device *dev)
{
	struct fwnode_handle *child;
	unsigned int ad4134_count = 0;

	device_for_each_child_node(dev, child) {
		if (fwnode_property_match_string(child, "compatible", "ad4134") >= 0)
			ad4134_count++;
	}
	return ad4134_count;
}
...
static int ad4134_probe(struct spi_device *spi)
{
	...
	if (ad4134_get_ADC_count(dev->parent) == 2) {
		...
		indio_dev->num_channels = ARRAY_SIZE(st->chip->chan_spec);
